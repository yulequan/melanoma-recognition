...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // vector<int> label_shape(batch_size,1, );
...
void ImageSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  vector<int> top_shape = this->data_transformer_->InferBlobShape(cv_img);
  this->transformed_data_.Reshape(top_shape);
  int origin_channel = top_shape[1];
  top_shape[1] = 1;
  this->transformed_label_.Reshape(top_shape);
  
  // Reshape batch according to the batch_size.
  top_shape[0] = batch_size;
  top_shape[1] = origin_channel;
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    cv::Mat cv_seg = ReadImageToCVMatNearest(root_folder + lines_[lines_id_].second,
              new_height, new_width, false);
    CHECK(cv_seg.data) << "Could not load " << lines_[lines_id_].first;
    ...
    this->data_transformer_->TransformImgAndSeg(cv_img_seg, &(this->transformed_data_), &(this->transformed_label_),
                            ignore_label);
